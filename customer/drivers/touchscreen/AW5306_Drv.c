...
short	BIGAREA_PEAK_TH = 80;
short	THPEAK = 60;	//suspected touch threshold
short	THGROUP = 70;	// touch threshold
	...
	if(readNum > 0)
	{
		...
		for(i=0;i<AWTPCfg.TX_LOCAL;i++)
		{
			for(j=0;j<AWTPCfg.RX_LOCAL;j++)
			{
				...
				if ((i == 0)||(i == AWTPCfg.TX_LOCAL-1)||(j == 0)||(j == AWTPCfg.RX_LOCAL-1)&&(AW_Cali.FirstFlag == 0))  //Edit By lizhen20130302
					{
					 //printk("BeforeDiff[i=%d][j=%d] = %d\n",i,j,Diff[i][j]);
					 Diff[i][j] = Diff[i][j]*3/5;
					 					 //printk("AfterDiff[i=%d][j=%d] = %d\n",i,j,Diff[i][j]);
					}
				else if ((j == 1)&&(AW_Cali.FirstFlag == 0))  //Edit By lizhen20130302
					{
					 if ((Diff[i][j-1]>100)&&(Diff[i][j-1]>2*Diff[i][j]))
					 	{
					 		Diff[i][j] = Diff[i][j] + 38;
					 	}
					 	else
					 	{
					 		;
...
void  FilterMarginPoint(void)
{
    ...
    //printk("FiltMargin X11111 = %d,Y11111 = %d\n",x,y);    //edit by lizhen20130302 for MarginFilt
    if ((x<=55)&&(x>=35))
    	{
    		temp = x/2+28;
    		x = temp ;
		...
		//printk("FiltMargin X22222 = %d,Y22222 = %d\n",x,y);
