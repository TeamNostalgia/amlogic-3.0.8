...
void AW5306_CAC_CLB(void)
{
	...
	if (AveScreen == 0)
	{
		...
		for (Col = 0; Col < AWTPCfg.RX_LOCAL; Col++)
		{
			...
			if ((Col == 0)||(Col == 9))     //Edit By lizhen20130302
				  { 
				  	//printk("CaliCACBefore RxCac[Col] = %d ,Col = %d ",RxCac[Col],Col);
				    RxCac[Col] = RxCac[Col] +16;
				    //printk("CaliCACAfter RxCac[Col] = %d ,Col = %d ",RxCac[Col],Col);
				  }
			else 
				  {
					;
