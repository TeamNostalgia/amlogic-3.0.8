/*
	...
	History:
		...
		2012/02/10 Added muti_touch key.
				   い耞篨夹
		2012/04/02  input_report_key(android 4.0 event inputΤ暗э)
	...
	struct i2c_client *client, pm_message_t mesg)
{
	...
	//msleep(10000);
	ilitek_i2c_read(client,ILITEK_TP_SET_SLEEP,&buffer,1);
	return 0;
}
...
{
	struct  ilitek_platform_data *pdata = client->dev.platform_data;
        if(i2c.valid_irq_request != 0){
                ilitek_i2c_irq_enable();
        }
	else{
...
ilitek_i2c_register_device(
	void)
{
	struct  ilitek_platform_data *pdata = i2c.client->dev.platform_data;
	...
	if(ret == 0){
		...
		pdata = i2c.client->dev.platform_data;
		if(i2c.client->irq != 0 ){ // == => polling mode, != => interrup mode  IRQF_DISABLED IRQF_TRIGGER_FALLING RISING
			...
			if(i2c.irq_work_queue){
				...
				#ifdef CLOCK_INTERRUPT
				if(request_irq(i2c.client->irq, ilitek_i2c_isr, IRQF_DISABLED, "ilitek_i2c_irq", &i2c)){
					...
					if(pdata->init_gpio){
						printk(ILITEK_ERROR_LEVEL "ilitek_init_gpio\r\n");
						pdata->init_gpio();
					}
