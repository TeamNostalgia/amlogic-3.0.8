...
#if defined (CONFIG_MACH_MESON6_G11_E8)
    #include "t3data.h"
#else
...
static struct tc103_timer_data *tc103_priv_data = NULL;

static void tc103_start_timer(unsigned long delay)
{
    mutex_lock(&tc103_priv_data->lock);
    if (tc103_priv_data->timer_en == 0) {
        tc103_priv_data->timer.expires = jiffies + delay;
        tc103_priv_data->timer.data = tc103_priv_data;
        add_timer(&tc103_priv_data->timer);
        tc103_priv_data->timer_en = 1;
    }
    mutex_unlock(&tc103_priv_data->lock);
}

static void tc103_stop_timer(void)
{
    mutex_lock(&tc103_priv_data->lock);
    if (tc103_priv_data->timer_en == 1) {
        del_timer_sync(&tc103_priv_data->timer);
        cancel_work_sync(&tc103_priv_data->work);
        tc103_priv_data->timer_en = 0;
    }
    mutex_unlock(&tc103_priv_data->lock);
}

static void tc103_timer_func(unsigned long data)
{
    struct tc103_timer_data *pdata = (struct tc103_timer_data *)data;
    schedule_work(&pdata->work);
}

static void tc103_work_func(struct work_struct *work)
{
    /* tc103 data reovery to normal value */
    unsigned char tData[2] = {0x47, 0x99};
    twx_i2c_write(twx_tc103_bl_i2c_client, tData, 2);
    //printk("recovery tc103 data to normal\n");
}

static int init_tc103_timer(void)
{
    tc103_priv_data = (struct tc103_timer_data*)kzalloc(sizeof(struct tc103_timer_data), GFP_KERNEL);
    if (!tc103_priv_data) {
        printk(KERN_ERR "tc103 driver data allocation failed\n");
        return -ENOMEM;
    }

    init_timer(&tc103_priv_data->timer);
    tc103_priv_data->timer.function = tc103_timer_func;
    tc103_priv_data->timer.data = tc103_priv_data;

    INIT_WORK(&tc103_priv_data->work, tc103_work_func);
    mutex_init(&tc103_priv_data->lock);
    return 0;
}

static unsigned int delay = 2000;
static ssize_t tc103_timer_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", delay);
}

static ssize_t tc103_timer_delay_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	...
	delay = (unsigned int)data;
    printk("delay is %d\n", delay);
	return count;
}

void tc103_set_timer(int enable)
{
    if (tc103_priv_data && twx_tc103_bl_i2c_client) {
        if (enable) {
            tc103_start_timer(msecs_to_jiffies(delay));
        } else {
            tc103_stop_timer();
        }
        //printk("%s tc103 timer\n", enable?"start":"stop");
    }
}
//////////////////////////////////////////////////////////////////////////
 ...
 void T3_PWMBL_CTR(int vla, int state)
{
    ...
    if(enable3d==0) {
        ...
        if(blvalue < BACKLIGHT_MIN_VALUE) 
            blvalue=BACKLIGHT_MIN_VALUE;
    } else {
        if(blvalue<BACKLIGHT_BASE_STEP) 
            blvalue=BACKLIGHT_BASE_STEP;
    }
...
static struct class_attribute enable3d_class_attrs[] = {
	...
	__ATTR(timer_delay,  S_IRUGO | S_IWUSR, tc103_timer_delay_show,    tc103_timer_delay_store), 
    __ATTR_NULL
};
