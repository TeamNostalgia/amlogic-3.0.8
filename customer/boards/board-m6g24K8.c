...
static int gc2035_v4l2_init(void)
{
	pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	gpio_out(PAD_GPIOE_11, 1);    // set camera power disenable
    gpio_out(PAD_GPIOE_10, 1);
	msleep(100);
	gpio_out(PAD_GPIOE_10, 0);
	msleep(100);
    printk( "amlogic camera driver: gt2035_v4l2_init. \n");
    gpio_out(PAD_GPIOZ_0, 0);
	msleep(100);
    gpio_out(PAD_GPIOZ_0, 1);
	msleep(100);
...
static struct ts_platform_data_ssd253x ssd253x_ts_pdata = {
	.irq_no 			= FT_IRQ,
...
static struct i2c_board_info __initdata aml_i2c_bus_info_a[] = {
...
#ifdef CONFIG_SSD25XX_CAPACITIVE_TOUCHSCREEN
    {
        ...
        .platform_data = (void *)&ssd253x_ts_pdata,
    },
#endif
};
...
//	WRITE_CBUS_REG( PAD_PULL_UP_REG0, READ_CBUS_REG(PAD_PULL_UP_REG0) & ~(1<<31) );
	...
	/* set input mode */
	//aml_set_reg32_bits(P_PREG_PAD_GPIO0_EN_N, 1, 15, 1);    // mode hq
	
      // gpio_enable_edge_int(gpio_to_idx(GPIO_SENSOR), 1, 2);
       // gpio_irq_set(PAD_GPIOA_15,GPIO_IRQ(1,GPIO_IRQ_RISING)); //hq
