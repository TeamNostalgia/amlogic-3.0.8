...
static int gc0308back_init(void)
{
    pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
...
static int gc0308back_v4l2_init(void)
{
    gc0308back_init();
}
static int gc0308back_v4l2_uninit(void)
{
    printk( "amlogic camera driver: gc0308back_v4l2_uninit. \n");
    gpio_out(PAD_GPIOE_11, 1);    // set camera power disable
    msleep(5);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}

static struct aml_camera_i2c_fig1_s gc0308back_custom_init_script[] = {
    {0x14,0x11},  //0x10  11
    {0xff,0xff},
};

static aml_plat_cam_data_t video_gc0308back_data = {
    .name="video-gc0308",
    .video_nr=0,//1,
    .device_init= gc0308back_v4l2_init,
    .device_uninit=gc0308back_v4l2_uninit,
    .custom_init_script = gc0308back_custom_init_script,
};
CONFIG_CAM_DEV(gc0308back, gc0308_i2c, CAM_GC0308);
#endif
...
static u8 gt82x_config[] = {
    0x0F,0x80,/*config address*/
		0x1D,0x0E,0x1C,0x0D,0x1B,0x0C,0x1A,0x0B,0x19,0x0A,0x18,0x09,0x17,0x08,0x16,0x07,0x15,0x06,0x14,0x05,0x13,0x04,0x12,0x03,0x11,0x02,0x10,0x01,0x0F,0x00,0x00,0x0A,0x01,0x0B,0x02,0x0C,0x03,0x0D,0x04,0x0E,0x05,0x0F,0x06,0x10,0x07,0x11,0x08,0x12,0xFF,0xFF,0x0B,0x03,0x10,0x00,0x00,0x20,0x00,0x00,0x04,0x00,0x00,0x02,0x40,0x30,0x34,0x03,0x00,0x05,0x00,0x02,0x58,0x04,0x00,0x49,0x45,0x44,0x40,0x0A,0x00,0x24,0x19,0x03,0x14,0x10,0x02,0xDA,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01
	};
...
static void bt_device_init(void)
{
    ...
    /* BT_HOST_WAKE */
    aml_clr_reg32_mask(P_PAD_PULL_UP_REG4,1<<10);
}
