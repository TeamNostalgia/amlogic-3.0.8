...
static void wm8960_i2c_shutdown(struct i2c_client *client)
{
    struct snd_soc_codec *codec = wm8960_codec;
           ...
           // snd_soc_update_bits(codec,48, ~0, 0x00A);// JD2 used for Jack Detect Input 
     printk("Amlogic <> %s(%d)\n", __func__,__LINE__);
#if 1	
 /* Mute Speaker */  	
 ...
 /*Power down DAC*/  	
 //snd_soc_update_bits(codec, WM8960_POWER2, ~0, 0);	
// snd_soc_update_bits(codec, WM8960_POWER2, 0x60, 0x60);	
 snd_soc_write(codec, WM8960_RESET, 0x000);//  Reset WM8960
 #endif	          
     snd_soc_update_bits(codec,24, 0x040, 0);// HPDETECT LOW = Speaker 
            //snd_soc_update_bits(codec,23, ~0, 0x1D1);//  Enable slow clock (TOEN) 
