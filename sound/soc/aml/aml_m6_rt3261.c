    ...
    struct snd_pcm_hw_params *params)
{
    ...
    ret = snd_soc_dai_set_sysclk(codec_dai, 0, params_rate(params) * 256 * 2, SND_SOC_CLOCK_IN);
    if (ret < 0) {
    ...
    ret = snd_soc_dai_set_sysclk(cpu_dai, 0, params_rate(params) * 256 * 2, SND_SOC_CLOCK_OUT);
    if (ret < 0) {
