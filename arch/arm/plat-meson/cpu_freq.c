...
//    }
#endif
     if (target_freq < meson_min_frq)
         target_freq = meson_min_frq;
...
int meson_lock_min_cpufreq(unsigned int min_freq)
{
    int ret = 0;
    int cur_cpu_frq = meson_cpufreq_get(0);
    mutex_lock(&meson_cpufreq_mutex);
    if (cur_cpu_frq < min_freq)
    	ret = meson_cpufreq_target_locked(NULL, min_freq, 0);
    meson_min_frq = min_freq;
    mutex_unlock(&meson_cpufreq_mutex);
    return ret;
}
