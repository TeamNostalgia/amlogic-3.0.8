...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	unsigned int old_freq_target = this_dbs_info->requested_freq;
#endif
...
#if ENABLE_BOOST
	if (change_skip_samples != -1 && change_skip_samples < dbs_tuners_ins.change_skips && !dbs_tuners_ins.boost_enable) {
#else
	if (change_skip_samples != -1 && change_skip_samples < dbs_tuners_ins.change_skips) {
#endif
	...
	if (max_load > 95 ||
		...
		if ((max_load > 95 || max_load_long > 95) &&
				early_suspend < DEF_EARLY_SUSPEND_SAMPLES && !dbs_tuners_ins.boost_enable)
#else
		if ((max_load > 95 || max_load_long > 95) && early_suspend < DEF_EARLY_SUSPEND_SAMPLES)
#endif
			this_dbs_info->requested_freq = policy->max;
		else {
			
#if ENABLE_BOOST
			if (early_suspend < DEF_EARLY_SUSPEND_SAMPLES && !dbs_tuners_ins.boost_enable)
#else 
		...
		if (early_suspend >= DEF_EARLY_SUSPEND_SAMPLES &&
				...
				policy->cur != dbs_tuners_ins.screen_off_max && !dbs_tuners_ins.boost_enable) {
#else
		if (early_suspend >= DEF_EARLY_SUSPEND_SAMPLES &&
				this_dbs_info->requested_freq > dbs_tuners_ins.screen_off_max &&
		...
		if (dbs_tuners_ins.boost_enable) {
			if (!allow_boost) {
				if (old_freq_target - dbs_tuners_ins.boost_off_max_freq >= (dbs_tuners_ins.freq_step * policy->max) / 100) 
					this_dbs_info->requested_freq = old_freq_target - (dbs_tuners_ins.freq_step * policy->max) / 100;
				else if (old_freq_target <=  dbs_tuners_ins.boost_off_max_freq) 
					return ;
				else 
			...
			if (old_freq_target == this_dbs_info->requested_freq)
			return ;
		}
#endif //ENABLE_BOOST
