...
#define VM_RES0_CANVAS_INDEX 0x70
#define VM_RES0_CANVAS_INDEX_U 0x71
#define VM_RES0_CANVAS_INDEX_V 0x72
#define VM_RES0_CANVAS_INDEX_UV 0x73

#define VM_RES1_CANVAS_INDEX 0x74
#define VM_RES1_CANVAS_INDEX_U 0x75
#define VM_RES1_CANVAS_INDEX_V 0x76
#define VM_RES1_CANVAS_INDEX_UV 0x77
...
int get_canvas_index(struct amlvideo2_output* output, int start_canvas)
{
...
static int amlvideo2_fillbuff(struct amlvideo2_fh *fh, struct amlvideo2_node_buffer *buf, vframe_t *vf)
{
	...
	switch(magic){
		case  MAGIC_RE_MEM:
#ifdef MUTLI_NODE
			output.canvas_id =  get_canvas_index(&output,(node->vid==0)?VM_RES0_CANVAS_INDEX:VM_RES1_CANVAS_INDEX);
#else
			output.canvas_id =  get_canvas_index(&output,VM_RES0_CANVAS_INDEX);
#endif
			break;
		case  MAGIC_VMAL_MEM:
...
static int amlvideo2_close(struct file *file)
{
	...
	amlvideo2_frmintervals_active.denominator = 20;
	//node->provider = NULL;
	mutex_unlock(&node->mutex);
	return 0;
}
