    ...
    )
{
    ...
    case PIN_BGF_EINT:
        if (PIN_STA_INIT == state) {
			...
			iRet = request_irq(mtk_bgf_irq,irq_handler,IORESOURCE_IRQ | IORESOURCE_IRQ_HIGHEDGE, "BT_wu_host",NULL);// wenhui modified
    ...
    )
{
    switch(state)
    {
	    ...
	    /* second: enable bgf irq wake up host function*/
            do {
                int iret;
                iret = enable_irq_wake(mtk_bgf_irq);//enable bgf irq wake up host function
	    ...
	    /* first: disable bgf irq wake up host function*/
            do {
                int iret;
                iret = disable_irq_wake(mtk_bgf_irq);//disable bgf irq wake up host function
                if (iret) {
