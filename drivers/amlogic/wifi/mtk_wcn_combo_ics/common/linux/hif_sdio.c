...
wifi_on_exist:
    ...
    if (!--func->card->host->sdio_irqs) {
		mtk_sdio_thread=func->card->host->sdio_irq_thread;
        HIF_SDIO_DBG_FUNC("kthread_stop sdioirqd\n");
	}

    if (!func->card->host->sdio_irqs++) {
        func->card->host->sdio_irq_thread =kthread_run(mtk_sdio_irq_thread, func, "mtksdioirqd");
        HIF_SDIO_DBG_FUNC("kthread_run mtk_sdio_irq_thread\n");       
        if (IS_ERR(func->card->host->sdio_irq_thread)) {
		int err = PTR_ERR(func->card->host->sdio_irq_thread);
		func->card->host->sdio_irqs--;
		return err;
	}
   }
...
#ifdef CONFIG_MTK_OOB_ONLY         
        if(mtk_sdio_thread){
            func->card->host->sdio_task_state = 0;
        kthread_stop(mtk_sdio_thread);
        mtk_sdio_thread = NULL;
        HIF_SDIO_INFO_FUNC("mtk_sdio_thread stop success!\n");
       if(hif_host)
        hif_host = NULL;   
        }
#endif
        ...
        kthread_stop(mtk_sdio_thread);
             mtk_sdio_thread = NULL;
            HIF_SDIO_INFO_FUNC("mtk_sdio_thread release success!\n");
        }
#endif 
        HIF_SDIO_INFO_FUNC("ok!\n");
...
wifi_off_exist:
    ...
    if ( (clt_index = g_hif_sdio_probed_func_list[probe_index].clt_idx) >= 0 )    /* the function has been registered */
    {
        ...
        sdio_release_host(func);
        if (!--func->card->host->sdio_irqs) {
		atomic_set(&func->card->host->sdio_irq_thread_abort, 1);
