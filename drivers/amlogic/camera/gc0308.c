...
//25M mclk
	...
	{0x60,0x07},
	{0x61,0x15},
	{0x62,0x0f}, // 0x08 betty
	{0x64,0x01},  // 0x03 betty
	{0x66,0xe8},
	{0x67,0x86},
	{0x68,0xa2},
	{0x69,0x18},
...
void set_GC0308_param_exposure(struct gc0308_device *dev,enum camera_exposure_e para)//曝光调节
{
	...
	switch (para)
	{
		...
		case EXPOSURE_0_STEP:
			buf1[0]=0xb5;
			buf1[1]=0x00;//08 betty
			buf2[0]=0xd3;
			buf2[1]=0x50;//50 betty
			break;
			...
			const struct i2c_device_id *id)
{
	...
	err=i2c_get_byte_add8(client, 0x00);
	printk("----------- GC0308 ID 0x%x -----------\n", err);
	plat_dat->device_uninit();
	if(err != 0xa0) {
		printk("--------------- gc0308 cannot find ------------------\n");
		return  -ENODEV;
	} else {
		printk("--------------- gc0308 find ------------------\n");
	}
